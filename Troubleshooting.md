...
Berkas ini memuat dua hal. Bagian pertama, **Masalah Umum & Solusi**, adalah
panduan untuk masalah yang berulang. Bagian kedua, **Troubleshooting Log**,
adalah catatan insiden bertanggal beserta akar masalah dan perbaikannya.

Tiga masalah tersering pada pemasangan baru ada di
[README.md](README.md#pemecahan-masalah).

## Masalah Umum & Solusi

### Composer: "Could not find a matching version of package"

Periksa dulu nama paketnya benar — misalnya `clue/redis-react`, bukan
`clue/reactphp-redis`. Jalankan Composer lewat container, bukan dari host:

```bash
sudo bash scripts/cbt.sh app composer require <nama-paket>
```

### WebSocket terputus periodik di belakang Cloudflare

Gejala: tidak ada pesan galat, tetapi Cloudflare Tunnel memutus koneksi
WebSocket setelah kira-kira 100 detik tanpa aktivitas.

Sistem sudah memiliki heartbeat 30 detik; daemon mengirim ping ke seluruh klien.
Bila pemutusan masih sering terjadi, periksa apakah loop timer di
`WebSocketServe.php` benar-benar tereksekusi dan tidak diblokir pemrosesan
sinkronus. Koneksi PDO ke MariaDB di dalam daemon adalah penyebab paling umum:
ia memblokir event loop sehingga heartbeat tidak pernah terkirim. Jangan
memakai PDO sinkronus di dalam daemon WebSocket.

### Progress jawaban tidak tersimpan

Seluruh autosave ditulis ke Redis (`exam_answers:attemptId`) selama ujian
berlangsung, lalu dipindahkan ke MariaDB secara massal di akhir. Bila Redis
mati, autosave ikut gagal.
...
### Sesi cepat berakhir atau sesi menumpuk

Pastikan session handler di `src/.env` tidak ter-override ke handler berkas.
Untuk Docker, gunakan Redis:
...
Sesuaikan nama host bila prefix container Anda berbeda dari bawaan.

### PHP-FPM kehabisan sumber daya, server terasa lambat

Terjadi bila pemasangan masih menyisakan jalur Server-Sent Events lama dan
daemon WebSocket belum sepenuhnya dipakai. Buka tab Network di DevTools browser
saat ujian berlangsung. Yang benar adalah satu koneksi ke `wss://domain/ws/`
dengan status `101 Switching Protocols`. Bila yang terlihat adalah permintaan
yang terus menerus berstatus *pending*, itu sisa SSE yang masih menahan worker
PHP-FPM.
