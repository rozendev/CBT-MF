...
- **Environment ini pakai `sudo bash scripts/cbt.sh <cmd>` sebagai satu-satunya cara menjalankan `composer`/`php` di container `ex_php`** (bukan `docker compose exec` langsung) — ini konvensi proyek, ikuti persis **kalau kerja langsung di checkout utama**.
- **Kalau plan ini dieksekusi di git worktree terpisah** (seperti biasanya dilakukan `subagent-driven-development`): `ex_php` di-mount ke folder checkout utama, bukan ke worktree — jadi `scripts/cbt.sh` TIDAK bisa dipakai (akan menguji kode yang salah). Gantikan setiap `sudo bash scripts/cbt.sh composer <args>` dengan `docker run --rm -v "$(pwd)/src:/var/www/html" -w /var/www/html cbt-mf-php composer <args>`, dan setiap `sudo bash scripts/cbt.sh php <args>` dengan `docker run --rm -v "$(pwd)/src:/var/www/html" -w /var/www/html cbt-mf-php php <args>` (dijalankan dari root worktree). Image `cbt-mf-php` sudah ada secara lokal (dibangun oleh `docker compose` di checkout utama), container-nya sekali-pakai (`--rm`) dan tidak menyentuh stack live.
